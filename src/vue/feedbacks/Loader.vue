  ...
  switch (currentStep.label) {
    ...
    case "loading":
      const durationPercentage = Math.min(
        Math.max((100 * elapsedTime) / currentStep.minDuration, 0),
        100
      );
      totalPercentage.value = Math.round(
        Math.min(durationPercentage, taskProgressPercentage.value)
      );
...
const _didReachStep = (animationStepLabel) => {
  const step = ANIMATION_STEPS.find(
    (step) => step.label === animationStepLabel
  );
...
const _isRunning = () => {
  return (
    currentStepId.value >= 0 && currentStepId.value < ANIMATION_STEPS.length
  );
};
